...
function analyzeIssue(issue, callback){
    ...
    var toSend = {"issue" : issue}
    
    //The url we want is `www.nodejitsu.com:1337/`
    var options = {
      host: 'alexa-therapy.herokuapp.com',
      path: '/test',
    ...
    //   port: '5000',
      ...
      headers: {
        ...
        'Content-Length': JSON.stringify(toSend).length
      }
    };
    ...
    var c = function(response) {
      ...
      response.on('end', function () {
        ...
        if(parsed && parsed.response){
            speechOutput = parsed.response;
            callback({}, buildSpeechletResponse("Therapy Response", speechOutput, repromptText, false));    
        } else {
            speechOutput = "Failed to process";
            callback({}, buildSpeechletResponse("Therapy Response", speechOutput, repromptText, false));    
        }
      });
    }
    
    var req = http.request(options, c);
